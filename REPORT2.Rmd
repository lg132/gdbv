...
For all queries `measure` was set to `"tonnage"`. The queried data was put into `data.frames` and saved locally serving as database for the Shiny Web App. Below an exemplary code is shown: 
...
# loop to get all entries of list into initial data.frame
...
In the second argument of the shiny app the _server_ is defined, being the part which reacts to the varying input variables and which compiles the output. First, two reactive functions, `dataInput` and `calcTable`, return the changing data which are needed to produce the outputs. Then, the outputs (plot, table and map) are created. Therefore, the returned changed input data is further processed for the `output$areaPlot` via an `if else` statement and plotting commands using `ggplot()`. The return of the `calcTable`-function gets passed to the `output$valuesTable` to create the respective table. In a last step the map is created, also reacting to the varying `dataInput` return. 
The Shiny App is run via the command `shinyApp(ui = ui, server = server)`.

# Results

The seaaroundus-Shiny Web App-R document is attached to the project file "seaaroundus" and can be opened and run via RStudio.
