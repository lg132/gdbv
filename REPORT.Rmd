...
In the main Panel defines the arrangement of the output in Tabs, i.e. showing either th plot *or* the table.
...
Then, both outputs, plot and table, are created. Therefore, the returned changed input data is further processed for the `output$areaPlot` via `if else` statements and plotting commands using `ggplot()`. The `output$values` to create the table is getting passed the return of the `calcTable`-function. The Shiny App is run via the command `shinyApp(ui = ui, server = server)`.
